...
State lives in two structures on `Exploration`: `active_groups_`, an `unordered_map<int, ExplorationGroup>` keyed by group ID; and `variant_tracking_map_`, which maps a variant's tracking ID to `{group_id, variant_index}` so `feedResult` can do constant-time routing. `FLASHIda` also maintains an atomic `exploration_active_` flag (`FLASHIda.h:234`) updated on every `processScan` boundary via `exploration_.activeGroupCount() > 0` (`FLASHIda.cpp:826, 865, 979, 1023`). The flag is intended for a lock-free scan-cycle gating check; as of today it is written but never loaded — a future hook, not a currently exercised one.
