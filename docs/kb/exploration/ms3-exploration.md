...
- **Non-exploration path only fires when MS3 is configured.** The call at `FLASHIda.cpp:927` is guarded by `config_.level(2).selection != SelectionMetric::None`, which effectively means "MS3 targeting is enabled". MS3 configuration is per-level and independent of whether MS2 exploration is enabled.
