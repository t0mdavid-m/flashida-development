...
- Quantification (`FLASHIda.cpp:395-414`, marker `'Q'`) — tag-independent, and after ADR-0038 no longer a "follow-up" at all: `ms_settings.ms2_quant` is ROSTERED per precursor and measured, and what it buys is the ordinary `'R'` identification scan. **Which verdicts buy is authored** (`quantification.identify`, ADR-0039): `differential` (the default, and ADR-0038's behaviour) | `quantified` | `all` | `none`, refined for the differential case by `quantification.enriched_in`. Deferred to a future quantification packet.
