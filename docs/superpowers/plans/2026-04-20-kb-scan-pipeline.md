...
- `buildFollowUp(const ScanCommand& ctx, const ScanConfig& follow_up_config, char suffix, int priority = 0)` (`:88`) — clones `ctx` with a fresh tracking ID and embeds a suffix letter after the encoded ID in `scan_description`.
