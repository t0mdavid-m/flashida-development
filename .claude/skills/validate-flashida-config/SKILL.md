...
| A17 | *retired by ADR-0044.* It required a non-empty `overrides` map under `remaining_precursor`, as the interlock for a scan-range binding that no longer exists; overrides are optional under every metric |
| A18 | `metric: "remaining_precursor"` paired with `"multiplexed"` **at the same level** — `precursor_charges` for the `precursor_selection` block, `fragment_charges` for the `characterization` one. The remaining-precursor ratio sums the **anchor's** window only, so under co-isolation it reports one charge state's depletion while the same CE depletes its siblings at other rates. Two level-matched checks, not one "multiplexed anywhere" test: `separate` and the **cross-level** pair both stay legal (ADR-0026 decision 4, re-grounded by ADR-0044) |
...
| A28 | `exploration.overrides.analyzer: "IonTrap"` with a metric other than `remaining_precursor` (ADR-0045). A trap pre-scan is **measured, never identified** — not deconvolved, not matched — so `mass_count` / `fragment_count` score every variant 0 and winner selection crowns `ce_min` every time: N scans per precursor to pick the first grid point, silently |
| A29 | `analyzer` outside the closed set `"Orbitrap"` \| `"IonTrap"` \| `""` (case-sensitive), at every scan object **and** every `overrides.analyzer` (ADR-0045). The engine branches on the string, so `"Iontrap"` would miss both gates and reach the instrument verbatim |
