...
def validate(path, ref):
    ...
    # Both keys are inert at their defaults (min 1, max 0), so every check here fires only on a
    # config that authored them. The error mirrors Config::validate(); the two warnings cover the
    # states the engine ACCEPTS and then ignores, which is this skill's whole reason to exist.
    min_cs = ps.get("min_charge_states", 1)
    max_cs = ps.get("max_charge_states", 0)
    pchg = ps.get("precursor_charges", "single")
    if isinstance(min_cs, int) and isinstance(max_cs, int):
        if max_cs > 0 and min_cs > max_cs:
            rep.error("A27", f"precursor_selection.min_charge_states ({min_cs}) exceeds "
                             f"max_charge_states ({max_cs}). The cap truncates the acquisition "
                             "charge set BELOW the floor, so every species is refused and the run "
                             "acquires nothing -- silently. Config::validate() throws on this.")
        if min_cs > 1 and pchg == "single":
            rep.warn("A27", f"precursor_selection.min_charge_states is {min_cs} but "
                            "precursor_charges is \"single\", where the acquisition charge set is "
                            "always size 1 -- so the floor is INERT and every species passes it. "
                            "Set precursor_charges to \"separate\" (one MS2 per charge, which is "
                            "what the quantification majority vote needs) or \"multiplexed\".")
        if max_cs > MAX_CHARGE_STATES_CEILING:
            rep.warn("A27", f"precursor_selection.max_charge_states is {max_cs}, above the "
                            f"instrument's {MAX_CHARGE_STATES_CEILING} windows per fragmentation "
                            "stage (MAX_NOTCHES_PER_STAGE + 1, an ABI dimension). The engine "
                            "clamps to that ceiling and the excess is silently ignored.")
