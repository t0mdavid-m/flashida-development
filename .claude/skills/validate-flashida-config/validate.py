...
PENDING_REFERENCE_KEYS = {
    # ADR-0040, Push 1. Legal on BOTH sides already (MethodConfig.cs + Config.cpp), but the
    # generated reference can only be regenerated by a C# test, so it lands one commit behind.
    # DELETE ALL THREE in the same change that promotes the regenerated reference.
    "precursor_selection.snr_threshold",
    "precursor_selection.max_charge_states",
    "precursor_selection.min_charge_states",
}
