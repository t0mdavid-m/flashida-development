...
The expected JSON output when `ToJSON()` is called on a `MethodParameters` loaded from `FlashIDA/src/Flash/etc/method.xml` (the default config). Generated by:
1. Running the test `P1-U03` locally.
2. Capturing the output.
3. Committing the file.

This file is the golden reference for the round-trip test.

**`FlashIDA/test-data/json/config_full.json`**

The expected JSON output when `ToJSON()` is called on `method_json_roundtrip.xml`. Same generation procedure. This exercises all array fields and non-default values.
...
|---------|----------------|-------------|-----------------|--------|
| P1-R01 | `regression-runner.ps1` with `method_default.xml` via JSON path | `Flash.exe -t` with JSON config active produces identical output to Phase 0 golden | `compare_golden.py baseline_phase0.tsv output.tsv` exits 0 | `windows-latest` |
| P1-R02 | `regression-runner.ps1` with legacy string override | `Flash.exe -t` with legacy format (auto-detect fallback) produces identical output | `compare_golden.py baseline_phase0.tsv output.tsv` exits 0 | `windows-latest` |
...
After all implementation steps are complete and CI passes, verify the working product manually:

**Verification 1: `Flash.exe -t` runs with JSON config**

```powershell
cd FlashIDA/src/Flash/bin/Debug
Flash.exe -t path\to\ms1_smoke_test.txt output.tsv path\to\method_default.xml
```

Expected: Exit code 0, `output.tsv` produced, content matches `baseline_phase0.tsv` within tolerance.

**Verification 2: Round-trip field match**

From the NUnit test output, confirm P1-U03 passes. This verifies that a specific non-default field value (e.g., `MinCharge=4`, `HCDEnergy=29`) survives the path:
...
From NUnit test output, confirm P1-I02 passes: `CreateFLASHIda` with a legacy token string returns a non-null pointer.

**Verification 4: No output change**

Confirm P1-R01 passes: `compare_golden.py` reports `PASS` when comparing `Flash.exe -t` output (JSON config path) against `baseline_phase0.tsv`.
...
- [ ] All 10 Phase 1 tests (P1-U01 through P1-R02) pass in CI.
- [ ] All 7 Phase 0 tests (P0-U01 through P0-R01) still pass in CI (no regression).
- [ ] `compare_golden.py` reports `PASS` for P1-R01 (JSON config output matches `baseline_phase0.tsv`).
- [ ] `compare_golden.py` reports `PASS` for P1-R02 (legacy fallback output matches `baseline_phase0.tsv`).
- [ ] `Flash.exe -t` runs to completion without unhandled exceptions.
