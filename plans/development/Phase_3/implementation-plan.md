...
| 29 | Phase 3 lesson #1 | **Verify blittable struct arithmetic independently.** When a plan specifies struct sizes, compute `sum of field sizes + alignment padding` before writing code. Largest-member alignment determines tail padding (e.g., struct with `double` aligns to 8 bytes). Char array sizes and nested struct arrays are the most common sources of arithmetic error. |
| 30 | Phase 3 lesson #2 | **OpenMS ClassTest: use `NOT_TESTABLE` for deferred stubs**, not `ABORT_IF(true)`. `ABORT_IF(true)` marks the section as failed and causes CTest to report the binary as failed. `NOT_TESTABLE` passes cleanly. `STATUS("msg")` prints a message but still requires passing assertions. |
| 31 | Phase 3 lesson #3 | **MSVC tools (`dumpbin`, `cl.exe`, `link.exe`) require `vcvars64.bat` on GitHub Actions.** `microsoft/setup-msbuild@v2` only adds MSBuild to PATH, not the full VS developer environment. Use `shell: cmd` and call `"C:\Program Files\Microsoft Visual Studio\2022\Enterprise\VC\Auxiliary\Build\vcvars64.bat"` before any MSVC tool. |
| 32 | Phase 3 lesson #4 | **`build-dlls` workflow in OpenMS repo auto-triggers on push to `flashida-v9-bridge`.** No manual Windows build needed. After pushing C++ changes, wait for `build-dlls` to succeed, then: `gh run download <run-id> -R t0mdavid-m/OpenMS -n selected-bin-artifacts`. Copy `OpenMS.dll` to `FlashIDA/dll/` and commit. |
| 33 | Phase 3 lesson #5 | **Stage new P/Invoke test activation across 2 commits** when adding bridge exports. Commit 1: C# code with new tests `[Ignore]`d and wrapper methods try/catch guarded. Commit 2 (after DLL update): remove `[Ignore]`, activate hard export verification. Shadow validation calls in processors should always remain try/catch guarded. |
| 34 | Phase 3 lesson #6 | **Test both sub-modes when a feature has variants.** Inclusion mode has strict vs non-strict; non-strict allows non-target fill-ins when no inclusion masses match the spectrum. Create separate config files for each variant (e.g., `method_inclusion.xml` + `method_inclusion_strict.xml`) rather than parameterizing a single test. |
