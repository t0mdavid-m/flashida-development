...
| 28 | Phase 3 impl note #1 | **Struct layout arithmetic correction:** Plan specified `activation_type[16]` in both `ScanCommand` and `IsolationStage`, giving 360 + 10×X = 1144 → X=78.4 (impossible). Fix: `IsolationStage.activation_type` enlarged to `char[32]` (80 bytes), `ScanCommand.activation_type` removed (redundant with per-stage field). Result: 344 + 10×80 = 1144. C# `MarshalAs` SizeConst for `ActivationType` is 32, not 16. |
| 29 | Phase 3 lesson #1 | **`ABORT_IF(true)` counts as test failure in OpenMS ClassTest framework.** Use `NOT_TESTABLE` for deferred/stub test sections. `NOT_TESTABLE` marks the section as intentionally untested and passes cleanly. Future phases should never use `ABORT_IF(true)` as a "skip" mechanism. |
| 30 | Phase 3 lesson #2 | **`dumpbin` is not on PATH in GitHub Actions Windows runners.** Must call `vcvars64.bat` first: `call "C:\Program Files\Microsoft Visual Studio\2022\Enterprise\VC\Auxiliary\Build\vcvars64.bat" >nul 2>&1` before running `dumpbin`. Use `shell: cmd` (not `powershell`) for this step to avoid quoting issues with `call`. |
| 31 | Phase 3 lesson #3 | **Non-strict inclusion mode allows non-target masses to fill remaining slots.** The C++ engine runs two selection phases: Phase 0 picks targets, Phase 1 fills with non-targets. With `StrictInclusion=false` and no matching targets in the spectrum, ALL results are non-target fill-ins. Tests asserting "only inclusion-list masses" must account for this. Created `method_inclusion_strict.xml` (StrictInclusion=true) to test strict behavior separately. |
| 32 | Phase 3 lesson #4 | **Pre-built DLL in `FlashIDA/dll/` must be updated before activating new P/Invoke tests.** New `[DllImport]` declarations throw `EntryPointNotFoundException` at runtime if the DLL doesn't export the function. Guard new bridge tests with `[Ignore]` until the DLL is rebuilt, or download from the `build-dlls` workflow in the OpenMS repo (`gh run download <id> -R t0mdavid-m/OpenMS -n selected-bin-artifacts`). Shadow validation calls in processors are safe because they're wrapped in try/catch. |
| 33 | Phase 3 lesson #5 | **The `build-dlls` workflow in the OpenMS repo runs automatically on push** to `flashida-v9-bridge`. It produces a `selected-bin-artifacts` zip containing `OpenMS.dll`, `OpenSwathAlgo.dll`, `Qt6Core.dll`, `Qt6Network.dll`, and `FLASHDeconv.exe`. Build takes ~5 min with ccache. No manual Windows build needed. |
| 34 | Phase 3 lesson #6 | **DLL export verification CI step should be warning-only until the DLL is committed.** The commit sequence is: (1) push C++ to OpenMS, (2) push C# to FlashIDA, (3) wait for `build-dlls`, (4) download and commit updated DLL, (5) then promote `dumpbin` check to hard failure. Making it a hard failure before step 4 blocks all Windows CI runs. |
| 35 | Phase 3 lesson #7 | **Struct size calculation must account for alignment padding.** When designing blittable structs for P/Invoke, compute sizes by hand: (a) sum raw field sizes, (b) round up to largest member alignment for struct tail padding, (c) verify with `static_assert` (C++) and `Marshal.SizeOf` (C#). Arrays of structs multiply the padded size, not the raw sum. |
| 36 | Phase 3 lesson #8 | **Golden file identity confirms shadow-only behavior.** `baseline_phase3.tsv` is byte-identical to `baseline_phase0.tsv`, proving that ProcessScan shadow calls (which are try/catch wrapped) and the new queue infrastructure have zero impact on existing acquisition output. This is the expected result for any phase that only adds infrastructure without changing scan routing. |
| 37 | Phase 3 lesson #9 | **Phase 3 delivered:** `IsolationStage` (80 bytes) and `ScanCommand` (1144 bytes) blittable structs with `static_assert`/`Marshal.SizeOf` verification. Priority queue (4 levels, mutex, MS1 fallback). 3 bridge exports (`ProcessScan` stub, `GetNextScanCommand` full dequeue, `GetNextTrackingId` monotonic). Shadow validation in IDA/FAIMS/Quant processors. `ScanFactory.BuildFromCommand()`. `ScanSchedulingConfig` + `ParameterOptimizationConfig` (Phase 1 deferrals). 18 new tests (4 C++ unit, 4 C# layout, 5 C# bridge integration, 2 stress, 2 deferred stubs, 1 CI-only). All 53 C# tests pass, all 3 C++ test binaries pass. |
