...
>
> ⚠️ **The idle path emits no AGC prescan, and `IsAgc` is NOT a drain sentinel** (ADR-0031). Step 5 used to fabricate a prescan as filler — and, because it also called `recordAGCTime()`, reset the very timer Step 1 reads, so `agc_interval_seconds` never governed the real cadence. Prescans now come from `agc_interval_seconds` alone (production default **1 s**; all 43 committed test configs pin it at `9999999` so goldens stay wall-clock independent). Priority 3 is the sentinel because `makeMS1()` sets it and every other caller overrides to 0 — MS2 is 2, MS3 is 1 — so a priority-3 MS1 *is* "Step 5 fabricated this". Pinned on both test paths — containers and CI: `FLASHIda_ProcessScan_test::only_the_idle_survey_is_emitted_at_priority_3` ∥ `IdleSurveySentinelTests`. A *scheduled* prescan can still arrive mid-drain; breaking on it truncates the drain.

> ⚠️ **An MS1 either DECIDES or it OBSERVES, and the drain has a Step 2b** (ADR-0042). A **monitor
> scan** is an MS1 acquired *during an exploration sweep* purely so the operator can watch the source
> — deconvolved and logged, but it selects nothing, excludes nothing, and no later decision reads
> anything it wrote. Marker `'M'`, `scan_type "monitor"`, priority **0** (3 is the idle sentinel; 1
> and 2 are the sweep's own lanes). Opt-in per exploration level:
> `precursor_selection.exploration.monitor_ms1` and `characterization.exploration.monitor_ms1`, each
> `{enabled, interval_ms}`, both off by default — so **no committed config emits one and no golden
> moves**. Three things make it safe rather than merely careful: `processScan`'s MS1 branch forks with
> **nothing after the fork**; the observing arm is a private **`static`** member, so `selection_` /
> `queue_` / `faims_` / `tracker_` are not nameable inside it; and it deconvolves through its own
> `monitor_deconv_`, never the shared `deconv_`. It must **not** call `recordMS1Time()` — a monitor
> scan satisfies `msn_level == 1 && is_agc == 0`, so without the `roleDecides` conjunct on the drain
> tail it stands in for a survey and starves `scheduling.cycle_time`, which is ADR-0031's bug again.
> `scan_description[3]` is now decoded in exactly one place, `FLASHIda/ScanRole.h`, whose traits table
> makes a new role declare whether it `decides`; an **undecodable** description resolves as a survey
> (both predicates true), which is what keeps the migrated read sites byte-identical.
...
- **FLASH code location** — headers under `OpenMS/src/openms/include/OpenMS/ANALYSIS/TOPDOWN/`, sources under `OpenMS/src/openms/source/ANALYSIS/TOPDOWN/` (the `source` tree has **no** `OpenMS/` segment — `source/OpenMS/…` does not exist). `FLASHIda.cpp` (real-time IDA driver; its `processScan` runs deconvolution + precursor selection) and `FLASHIdaBridgeFunctions.cpp` sit directly under `TOPDOWN/`; the 14 runtime helpers live in the nested `TOPDOWN/FLASHIda/` subdirectory: `Config`, `Deconvolution`, `Exploration`, `FAIMS`, `FragmentAnalysis`, `IdaLogger`, `MS3FragmentMatcher`, `Ms2Params`, `PrecursorSelection`, `ProteoformTracker`, `Quantification`, `ScanCommand`, `ScanCommandQueue`, `ScanRole`. A fifteenth file there never runs in real time: `ScanCommandJoin` (header-only) is the offline join FLASHDeconv uses to locate a commanded precursor from `scan_commands.tsv` (ADR-0046). See `OpenMS/CLAUDE.md` for what each owns.
...
- **Method configuration is JSON** — `FlashIDA/src/Flash/etc/method.json` (**not** XML). Top-level sections map to `[JsonKey]` classes in `FlashIDA/src/Flash/MethodConfig.cs` (note: directly under `src/Flash/`, there is no `Configuration/` directory): `global`, `deconvolution`, `precursor_selection`, `flashtnt`, `tagging`, `quantification`, `faims`, `ms_settings`, `scheduling`, `characterization`, `files`, `runtime`, plus a synthetic `developer` section into which `[Developer]`-marked fields are routed. Loading is reflection-driven (`FlashIDA/src/Flash/IDA/MethodConfigSerializer.cs`); FlashIDA then re-serializes to a *different* C++-facing schema via `MethodParameters.ToCppJson()` before crossing the bridge. See `docs/kb/config-flow/`.
  ...
  - **The charge set has an authored floor, an authored cap and an authorable SNR gate** (ADR-0040), all in `precursor_selection`, all inert by default. `min_charge_states` (default `1`) refuses a species whose acquisition charge set is smaller — inside `admitCandidate`, so it costs no `max_precursors` slot — and **exempts a sibling**, whose set is deliberately reduced by what the species already isolated (ADR-0036); without that exemption the floor refuses exactly the charge 0036 exists to recover. `max_charge_states` (default `0` = all) caps the set, keeping the most intense, and is translated to `AdmissionContext::max_notches` as **`max_charge_states - 1`** because the anchor is not a notch. `snr_threshold` (default `1.0`) was **hardcoded and stomped** — `Config.cpp` assigned it *after* the whole parse, so adding the key without deleting that line would have left it bound, documented and inert.
  ⚠️ **`separate` had no bound of its own until 0040, and the one it inherited was meaningless.** `buildMS2` writes notches only under `multiplexed` (`ScanCommandQueue.cpp:316`), yet `separate` was clamped by `MAX_NOTCHES_PER_STAGE` — the capacity of a wire array it never writes into. Measured on the `separate_charges` golden: **10 of 18 species-surveys sat at exactly 10 charges, and 100 of that mode's 135 MS2 scans came from them.** `min > max` is a load error: the cap would truncate below the floor, so every species would be refused, silently.
...
- `docs/adr/` — accepted architecture decisions, **forty-five files** spanning 0001–0046 (0006 is used twice; 0022 and 0024 are unused; 0017 is superseded by 0019, 0018 by 0021, 0032's submission threshold is amended by 0033, 0008's two-channel model is extended by 0035, 0028's per-survey species guard plus 0016's one-scan clause are amended by 0036, **0037 is withdrawn**, 0014's name-reference rule is taken the other way for one case by 0038, and 0038's "a differential verdict buys" clause is amended by 0039, and 0038's per-precursor screen plus 0016's acquisition-charge-set bounds are amended by 0040, and 0032's depth accounting is corrected by 0041, and 0031's recordMS1Time hazard plus 0008/0035's unsettled role-channel question are both taken up by 0042, and 0041's end-boundary is answered the other way at the START boundary by 0043, and 0026's window-bound scan range plus its overrides-mandatory rule are withdrawn by 0044, and 0020's every-MS2-variant-is-identified clause is narrowed for the ion trap by 0045, and 0035's decisions 7–8 plus its only-identity-that-survives-conversion claim are amended by 0046): direct-infusion precursor scope, ProteoformTracker dispatch authority, two-stage MS3 parameter sourcing, characterization config reshape, MS3-target-is-a-containing-fragment, single bridge config schema, winner-anchored fragment pooling, strict config-schema rejection, separate scan identity channels, scan-config-determines-instrument-parameters, positional stage arrays, source-region-parameters-are-survey-scoped, FAIMS-enablement-is-explicit, characterization-mode-is-the-single-MS3-switch, two-decision-sections-and-named-scan-configs, log-dir-is-resolved-host-side, co-isolated-charges-are-one-detection, notches-occupy-spare-stage-slots, charge-keyed-exclusion-is-a-fallback, notches-get-their-own-array-and-a-per-stage-cap, a-measuring-MS3-sweep-must-be-closed-by-a-follow-up, precursor-charges-is-the-only-acquisition-geometry, exhaustive-characterization-targets-unassigned-masses, the-drain-acquires-no-analysis-lock, a-remaining-precursor-sweep-scans-only-the-window-it-reads, identification-is-gated-by-the-sequence-not-the-MS3-switch, an-authored-charge-set-restricts-acquisition-and-re-keys-exclusion, a-baseline-belongs-to-its-activation, activation-decides-whether-a-coupled-parameter-is-emitted, agc-prescans-are-interval-scheduled-only, only-a-commanded-scan-earns-a-command, an-idle-instrument-acquires-its-own-method, flashdeconv-targets-the-toppic-1-8-feature-layout, ida-log-is-compatible-with-its-consumer-not-its-history, a-split-envelope-is-one-precursor-acquired-in-parts, a-matched-inclusion-target-is-barred-by-its-score, quantification-screens-and-identification-is-what-it-buys, the-quantification-objective-decides-what-a-verdict-buys, quantification-votes-across-charge-states, a-run-ends-where-its-acquisition-ends, a-monitor-scan-observes-the-source-and-decides-nothing, a-run-clock-starts-where-control-starts, a-pre-scan-reads-out-its-levels-scan-range, a-trap-pre-scan-is-measured-never-identified, a-commanded-scan-is-located-by-its-command-not-searched-for. Read the relevant ADR before re-litigating one of these.
  ⚠️ **0044 withdrew 0026's scan-range binding, and 0045 is what that made necessary** (both
  implemented 2026-09-22). A `remaining_precursor` pre-scan is no longer narrowed to its isolation
  window: it reads out its level's configured `first_mass`/`last_mass` like every other scan built
  from `scans[0]` + overrides, baseline and ADR-0023 forced sweep included. The premise — trap scan
  time is proportional to the range swept — did not survive the instrument: "~900x" was a ratio of
  range *widths*; only the readout part of a trap scan scales with the range (nominally ~14–54 ms
  for 1800 Th) and the rest does not shrink. 0026's overrides-mandatory rejection went with it (it
  was the narrowing's interlock; `method_exploration_ms3_remaining.json` is the ADR-0020 gate-#2
  case again), and the multiplexed rejection stays on the one ground that survives: the
  remaining-precursor ratio reads the **anchor's window alone**. **0045**: a trap pre-scan is now a
  real spectrum, and at MS2 every variant used to be deconvolved, matched and **pooled** under every
  metric — which the narrow range had made inert by accident. So a variant whose OWN command names
  the ion trap is **measured, never identified** (`isTrapAnalyzer(cmd.analyzer)` in
  `Exploration::feedResult` — the engine's first read of `ScanCommand::analyzer`); only
  `remaining_precursor` may score a trap sweep (any other metric scores every variant 0 and crowns
  `ce_min` — rejected at load); and `analyzer` is a closed set (`Orbitrap | IonTrap | ""`),
  validated on the raw JSON value at every scan object and `overrides.analyzer`. The
  `exploration_iontrap` golden (mode 29) is the first coverage a trap pre-scan has ever had; its
  fixtures are the **real** 2026-09-06 cytC sweep, centroided (`test-data/spectra/FIXTURES.md`),
  because the older `ms2_cytc_ce*` set gives a flat ratio and would crown a coin flip. **No FLASHIda
  run has yet acquired a trap pre-scan on hardware**, so whether the window sum holds on
  unit-resolution trap centroids, and whether the instrument clamps or refuses a `first_mass` below
  the trap's low-mass cutoff, are both still open there.
  ...
  ⚠️ **0039 makes "which verdicts buy" a config key; 0038's rule is now only the DEFAULT.**
  `quantification.identify` is a cut point on the verdict ladder — `differential` (the default, and
  0038's exact behaviour) | `quantified` (anything cleanly measured) | `all` | `none` (quantify
  only, never identify) — and `quantification.enriched_in` names the **condition** a differential
  species must be enriched in, or `"either"`. Never a direction: `fold_change =
  mean(conditions[0]) / mean(conditions[1])`, so `"up"` would have meant *enriched in control* on
  `method_quant.json` and would invert if the array were reordered.
  Because tagging and MS3 are suppressed on a `'Q'` and ride the bought `'R'`, `identify` also
  decides what gets **characterized** — with no MS3 code involved. A **fourth** throw joins the
  three above: `enabled` without `ms_settings.ms2`, which until 0039 built the bought scan from a
  default-constructed `ScanConfig` (the pre-existing guard reads the roster, and in a quant config
  the roster is non-empty because the `'Q'` holds it). `ms2` is required-but-inert under
  `identify: "none"`, so flipping the objective never invalidates a config; for the same reason an
  inert `enriched_in` is a `[CONFIG-WARN]`, not a throw.
  ⚠️ **`enriched_in` is evaluated on `condition_means`, NEVER on `fold_change`** — a wholly-absent
  condition is `Differential` with `fold_change == -1.0`, a *sentinel*, so a `fold_change`-based
  test silently drops the strongest results in the experiment. Pinned by
  `processScan_enriched_in_survives_a_wholly_absent_condition`, which needs **both** absence
  directions: with `conditions[0]` absent a bug spelled `fold_change < 1.0` coincidentally agrees
  with the correct answer, and only the mirror fixture separates them.
  ...
  **0037** proposed the *across*-survey half: a matched target would stop reading the two
  ...
  FLASHIda neither mints nor requests. It survives into the converted mzML — **and so does the
  tracking id**: msconvert writes the scan description as
  `<userParam name="scan description" value="!!BR5.30533k@10"/>`, which OpenMS loads as the
  spectrum meta value `"scan description"` (measured: 18,548 of 18,548 spectra of a real run). This
  file and ADR-0035 used to call the scan number "the only one that survives"; that was wrong, and
  ADR-0046 corrects it. The two join different things — the scan number places a spectrum in the
  acquisition's order, the tracking id ties it to the **command** that caused it. `ida.log`'s
  `MS1 Scan#` carries the scan number; `Access ID` on that same line keeps the **tracking id**,
  because that is the join key to the other four streams. The scan number reaches the engine as a
  trailing `int` on `ProcessScan` — **appended, never inserted**, so a stale 8-parameter
  `OpenMS.dll` ignores argument 9 rather than reading an `int` as `faims_cv`. Export count is still
  5 and `ScanCommand` is untouched. `<= 0` means "not supplied" and the log falls back to the
  tracking id.
  ⚠️ **FLASHDeconv no longer reads `ida.log` — it reads `scan_commands.tsv`** (ADR-0046,
  `-FD:scan_commands`; `-FD:ida_log` and the wizard's `IDALog_<base>.log` checkbox are **removed**).
  Each mzML MS2 is joined to its row by **tracking id**, and the row is an authoritative *locator*,
  not a mass source: it names the survey the command was decided from — by walking
  `parent_tracking_id` **up to the MS1 row**, because a follow-up MS2's parent is the MS2 that
  triggered it — and the mass, and FLASHDeconv takes **its own** PeakGroup for that mass from that
  survey. The reason is a measurement, not the ±0.001 window tolerance that started the work: 88 %
  of MS2 have 1–5 newer surveys between them and their parent survey, FLASHDeconv used to search
  the *latest* one, and its pick agreed with the commanded mass 40–46 % of the time against 95 %
  when it happened to read the right survey — while its deconvolution of the right survey contains
  the commanded mass for 99.8 % of MS2. The whole join is validated before any deconvolution and
  **fails closed**, because tracking ids restart every run and a foreign file joins 100 % of scans
  by id; a file that joins *nothing* aborts too, which is also what a converter that drops the scan
  description produces. MS2 only. The parse + join are pure functions in
  `FLASHIda/ScanCommandJoin.h` and are tested in `FLASHIda_Logging_test`; **the locate step inside
  `FLASHDeconvAlgorithm` is executed by no CI job** — CI only compiles `FLASHDeconv` and the wizard —
  so it is verified by an acceptance run, never by a green badge. The engine's one change is
  `scan_commands.tsv`'s `mono_mass` going from six significant digits to four decimals, which **no
  golden can see** (`RelTol 1e-3`): the 28 goldens were deliberately left alone and
  `scan_commands_mono_mass_is_written_at_four_decimals` is the only gate. ADR-0035's decision 7
  (scan number in `scan_results.tsv`) was **never implemented** and 0046 makes it unnecessary — do
  not go looking for that column. The wizard's old checkbox was **inert**: it wrote `ida_log` at the
  tool's top level while FLASHDeconv registers the key only under `FD:`, which TOPPBase answers
  with a warning.
