...
The build is **Release** (not Debug), Ninja, no GUI/pyOpenMS, static Boost, and after `--target OpenMS` it compiles **24 named FLASH test binaries**, not the whole library:
```bash
...
ctest --test-dir OpenMS/build -R "DeconvolvedSpectrum_OptimizationMetadata|FLASHIdaQueueTracking|FLASHIda_ProcessScan|ScanCommandLayout|FLASHIdaFAIMS|FLASHIda_exploration|FLASHIda_LegacyConfig|ConfigSchemaParity|FLASHIda_LoggingFields|FLASHIda_Logging|ScanCommandQueue_Concurrent|FragmentAnalysis|MS3FragmentMatcher|FLASHIda_ChargeBasedExclusion|FLASHIda_ChargeModes|ScanConfig_applyOverrides|Config_SchemaProjection|ProteoformTracker_CEOptimization|ProteoformTracker_Trajectory|ProteoformTracker_Localization|ProteoformTracker_WinnerContext|ProteoformTracker_NonWinnerRematch" --output-on-failure
# a single test
...
- `docs/adr/` — accepted architecture decisions, **twenty-one files numbered 0001–0020** (0006 is used twice; 0017 is superseded by 0019): direct-infusion precursor scope, ProteoformTracker dispatch authority, two-stage MS3 parameter sourcing, characterization config reshape, MS3-target-is-a-containing-fragment, single bridge config schema, winner-anchored fragment pooling, strict config-schema rejection, separate scan identity channels, scan-config-determines-instrument-parameters, positional stage arrays, source-region-parameters-are-survey-scoped, FAIMS-enablement-is-explicit, characterization-mode-is-the-single-MS3-switch, two-decision-sections-and-named-scan-configs, log-dir-is-resolved-host-side, co-isolated-charges-are-one-detection, notches-occupy-spare-stage-slots, charge-keyed-exclusion-is-a-fallback, notches-get-their-own-array-and-a-per-stage-cap, a-measuring-MS3-sweep-must-be-closed-by-a-follow-up. Read the relevant ADR before re-litigating one of these.
  ...
  ⚠️ **`precursor_charges: "separate"` was inert until it was fixed**, and the mechanism is worth knowing because it is a shape that recurs: the mode was implemented *only* as a suppressed `break` over `charges_to_process`, and that list is multi-valued **only** under `charge_based_exclusion` — an unrelated exclusion-keying developer flag that defaults to off. Acquisition GEOMETRY was being sourced from an exclusion-KEYING flag, so `separate` silently equalled `single` in every shipped config while CI stayed green: the one behavioural test configured the flag on. Both non-single modes now derive their charge set from the same `peakGroupNotchCandidates` + `selectNotches` pair (`NotchSelection.h`), which is what makes ADR-0016's "differ only in scan count" true rather than aspirational. Pinned by `FLASHIda_ChargeModes_test`.
